...
function injectDownloadFolderBtn() {
    ...
    if (isFold) {
        ...
        if (devider) {
            let _html = `
            <li class="d-block d-md-none EricKwok">
            ...
            <li class="d-block d-md-none dropdown-divider EricKwok" role="none"></li>`;
        ...
        if (html) {
            let _html = `
            <details data-view-component="true" class="details-overlay details-reset position-relative mr-2 EricKwok">
...
function removeAllInjectedElem() {
    document.querySelectorAll(".EricKwok").forEach(elem => elem.remove());
}
